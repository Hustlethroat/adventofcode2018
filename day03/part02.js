...
class Fabric {
  ...
  applyClaim(claim) {
    this.claimOverlaps[claim.id] = false;
      ...
      for (let yIndex in this.fabric[xIndex]) {
        ...
        if (vals && vals.length > 1) {
          for (let valIndex in vals) {
            this.claimOverlaps[vals[valIndex]] = true;
          }
        }
      }
    }
...
for (let overlapIndex in fabric.claimOverlaps) {
  if (!fabric.claimOverlaps[overlapIndex]) {
    console.log(`found ${overlapIndex}`);
  }
}
