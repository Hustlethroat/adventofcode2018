...
while (true) {
  ...
  if (currentDate.getHours() === 0 && isAsleep) {
    ...
    if (maxMinuteTimes < guardSleepCounts[currentGuard][minute]) {
      maxMinuteTimes = guardSleepCounts[currentGuard][minute] ;
      maxMinute = minute;
...
console.log(guardSleepCounts);


console.log(`${maxGuard} slept most at minute ${maxMinute} ${maxMinuteTimes} times, result ${Number(maxGuard) * Number(maxMinute)}`);
